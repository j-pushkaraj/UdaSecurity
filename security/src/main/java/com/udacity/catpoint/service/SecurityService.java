...
public class SecurityService {
    ...
    public void setArmingStatus(ArmingStatus armingStatus) {
        if(armingStatus == ArmingStatus.DISARMED) {
            ...
            // Reset all sensors to inactive when system is armed (copy set to avoid ConcurrentModificationException)
            Set<Sensor> copy = new HashSet<>(getSensors());
            copy.forEach(s -> {
                s.setActive(false);
                securityRepository.updateSensor(s);
            });
            // Transition to ALARM if armed home while camera shows a cat
    ...
    private boolean allSensorsInactive() {
        return securityRepository.getSensors().stream().allMatch(s -> !s.getActive());
    }
